...
struct GlobalInventoryView: View {
    @Environment(Router.self) private var router
    @Environment(GlobalInventoryViewModel.self) private var viewModel
                    ...
                    ScrollView(showsIndicators: false) {
                        LazyVStack(spacing: 16) {
                            ForEach(viewModel.filteredSummaries) { summary in
                                Button(action: {
                                    router.push(CARoute.inventoryDetail(summary))
                                }) {
